...
def greedy_2_regret_algorithm(matrix,instance):
    ...
    print(f"Start_a = {result_a}, start_b = {result_b}")
...
def perform_test(functions, instance_filenames):
    for function in functions:
        for instance_filename in instance_filenames:
            ...
            print(f"{function.__name__}, {instance_filename}, min={mini[0]}, maxi={maxi[0]}, avg={sum([a[0] for a in results])/len(results)}")
...
if testing:
    functions = [nearest_neighbor_algorithm,greedy_cycle_algorithm,greedy_2_regret_algorithm]
    instance_filenames = ['kroA100.tsp','kroB100.tsp']
    ...
    plt.title(f"Nearest neighbor. Start with {a[0]}=>{calc_cost(a, dist_matrix)}, with {b[0]}=>{calc_cost(b, dist_matrix)}")
    plt.show()
